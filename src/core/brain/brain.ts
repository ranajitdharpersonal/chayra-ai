...
export async function askBrain(prompt: string, systemInstruction: string = "You are an expert AI.", preferredEngine: string = "GEMINI"): Promise<BrainResponse> {
  ...
  if (preferredEngine === "GEMINI") {
    try {
      ...
      const res = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/gemini-3-pro:generateContent?key=${geminiKey}`, {
        method: "POST",
  ...
  // ==========================================
  // 2️⃣ PRIORITY 2: LLAMA 3.3 (Groq Fallback)
  // ==========================================
  ...
  // ==========================================
  // 3️⃣ PRIORITY 3: QWEN 2.5 (HuggingFace Final Lifeline)
  // ==========================================
  try {
